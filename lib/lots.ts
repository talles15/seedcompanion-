...
export async function fetchLotByNumero(numerolote: string): Promise<LotRow | null> {
  const supabase = createSupabaseServerClient();
  const { data, error } = await supabase
    .from("lots")
    .select(LOT_SELECT)
...
export function bestValue<T extends { data: string | null }>(
...
export function latest<T extends { round: number }>(rounds: T[] | null | undefined): T | null {
  if (!rounds || rounds.length === 0) return null;
  return [...rounds].sort((a, b) => b.round - a.round)[0];
}

export function byRound<T extends { round: number }>(rounds: T[] | null | undefined, round: number): T | null {
